    ...
    <h4 class="card-header"><a href="{{ route('LaravelCmsPages.show', ($sub_page->slug ?? $sub_page->id . '.html'), false ) }}">{{$sub_page->menu_title ?? $sub_page->title}}</a></h4>
      <div class="card-body">
        @if ( isset($img_width) )
            ...
            <img class="float-left mr-2 img-fluid img-thumbnail p-0 sub-page-img" src="{{$helper->imageUrl(json_decode($sub_page->file_data)->main_image, $img_width ,( $img_height ?? 'auto') ) }}" alt="{{$sub_page->title}}" />
            </a>
        @endif
