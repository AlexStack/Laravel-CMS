            ...
            <ul class="navbar-nav ml-auto ">
                ...
                <li class="nav-item
                 ...
                 <% if $isCurrent %>active<% end_if %>" >
                    @if ( count($item->menus) >0 )
                    <a class="nav-link dropdown-toggle" href="{{ $controller->url($item)  }}" id="drop{{$item->id}}" role="button" aria-haspopup="true" aria-expanded="false" data-toggle="dropdown" >{{ $item->menu_title ?? $item->title }}
                        ...
                        @if ( count($item->menus) >0 )
                            ...
                            @foreach ( $item->menus as $item2)
                                <a class="dropdown-item" href="{{  $controller->url($item2)  }}">
                        ...
                        @endif
                    @else
                        <a class="nav-link" href="{{ $controller->url($item)  }}">
...
{{--
    ...
    @if ( isset($file_data->main_banner) )
        <div class="text-center top-banner" style="min-height:100px; background-image: url({{$controller->imageUrl($file_data->main_banner, '2000') }});" ></div>
    @endif
</header>
